...
| **Q8** | **Admin "see everyone's activity" is a god-view** that itself needs auditing. | An operator reading members' activity is a sensitive action. | The admin surface stays a **separate route** gated by the `portal.admins` allow-list, consistent with [ADR 0016](0016-admin-portal.md) §6. Auditing the operator's *own* portal reads (and the step-up that gates the admin surface) is part of the [ADR 0016](0016-admin-portal.md) §6 admin-surface/step-up work — **not yet wired in Phase 0+**, and this ADR does not claim it is. The reads remain secret-free and the cross-person path is operator-only, so the residual gap is "the operator's read is not self-audited," tracked for the step-up phase. |
