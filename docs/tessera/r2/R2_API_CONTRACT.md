...
`POST /api/v1/capabilities/{id}/invoke` may add a coarse `stage` extension (`request`, `registry-account`, `registry-context`, `registry-capability`, `mcp-discovery`, `mcp-validation`, `plugin-create`, `execution`, `evidence`, or `message`) to `400 invalid_request`. It never includes exception text, input, identity, or credential material.
