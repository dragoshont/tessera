...
| SDD-01 **P4** | #1 | The real verdict engine: a periodic, rate-limited use-based liveness probe promoting `unverified → live/dead`, plus a **freshness bound** so a stale `lastVerifiedAt` decays off green. | **sign-off** | Planned. Probing live sessions (incl. the just-recovered RM) can itself trigger rate limits / re-auth — design + human approval before it runs against live upstreams. |
| SDD-02 | #2 | **Proactive degradation push** + **family-grade re-login hand-off** + **actionable chat errors** for failed brokered calls. | split | Planned. *Notification surface + structured error projection* = autonomous (additive portal/API). *Family-grade hand-off* (ADR 0016, noVNC recovery) touches live recovery = **sign-off**. |
| SDD-03 | #3, #6 | Resolve worker **dispatch-vs-sidecar**; add a **KV lease/CAS** that *enforces* single-rotator (A4 by construction → by runtime). | **plan-only** | Planned. KV lease + worker topology = infra/identity; agent proposes the IaC diff + policy, human applies. |
| SDD-04 | #4 | **Non-Azure golden path** (compose + OpenBao + Authentik/Dex); clarify the **Tessera↔sessionkeeper boundary**; one reusable **reference MCP**; a **recipe-contribution path**; explicit **adopt-vs-build + ToS posture**. | split | Planned. *Boundary docs, adopt-vs-build posture, recipe-contribution guide, ToS fencing* = autonomous (docs). *OpenBao/Authentik compose path* = **plan-only** (new infra). |
| SDD-05 | #5 | **Token freshness** — read-through-on-401 / short-lease — the prerequisite that makes the reverted v0.6.0 proxy cutover safe to re-attempt. | **sign-off** | Planned. Changes the broker's credential resolution against live upstreams and gates the R1 cutover — design + human approval. |
