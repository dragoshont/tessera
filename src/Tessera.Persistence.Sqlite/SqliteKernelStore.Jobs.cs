...
public sealed partial class SqliteKernelStore
{
    ...
    {
        await using var connection=await OpenConnectionAsync(token).ConfigureAwait(false);await using var command=connection.CreateCommand();command.CommandText="SELECT job_id,name,instruction,desired_state,health,model_profile_id,schedule_json,next_occurrence,context_policy_json,created_at,updated_at,version,kind,conversation_id FROM jobs WHERE owner_principal_id=$owner ORDER BY updated_at DESC,job_id;";command.Parameters.AddWithValue("$owner",owner);var values=new List<ProductJob>();
        await using(var reader=await command.ExecuteReaderAsync(token).ConfigureAwait(false)){while(await reader.ReadAsync(token).ConfigureAwait(false)){var schedule=JsonSerializer.Deserialize<JobSchedule>(reader.GetString(6))??throw new InvalidDataException("Job schedule is invalid.");values.Add(new(owner,reader.GetString(0),reader.GetString(1),reader.GetString(2),reader.GetString(3),reader.GetString(4),ReadNullableString(reader,5),schedule,ReadNullableTimestamp(reader,7),reader.GetString(8),[],[],[],ParseTimestamp(reader.GetString(9)),ParseTimestamp(reader.GetString(10)),reader.GetInt64(11),reader.GetString(12),ReadNullableString(reader,13)));}}
        for(var index=0;index<values.Count;index++)
        {var item=values[index];var accounts=await ReadJobGrantStringsAsync(connection,"job_account_grants","account_id",owner,item.JobId,token).ConfigureAwait(false);var effects=await ReadJobGrantStringsAsync(connection,"job_side_effect_grants","side_effect_class",owner,item.JobId,token).ConfigureAwait(false);var capabilities=await ReadJobCapabilitiesAsync(connection,owner,item.JobId,token).ConfigureAwait(false);var spec=item.Kind=="DEVELOPMENT"?await GetDevelopmentJobSpecAsync(owner,item.JobId,token).ConfigureAwait(false):null;values[index]=item with{AccountGrants=accounts,CapabilityGrants=capabilities,SideEffectGrants=effects,DevelopmentSpec=spec};}
    ...
    public async Task AddJobAsync(ProductJob job,CancellationToken token=default)
    {
        ...
        await using(var command=connection.CreateCommand()){command.Transaction=transaction;command.CommandText="""
            INSERT INTO jobs(owner_principal_id,job_id,name,instruction,desired_state,health,model_profile_id,schedule_json,next_occurrence,context_policy_json,created_at,updated_at,version,kind,conversation_id)
            VALUES($owner,$id,$name,$instruction,$state,$health,$profile,$schedule,$next,$context,$created,$updated,$version,$kind,$conversation);
            """;command.Parameters.AddWithValue("$owner",job.OwnerPrincipalId);command.Parameters.AddWithValue("$id",job.JobId);command.Parameters.AddWithValue("$name",job.Name);command.Parameters.AddWithValue("$instruction",job.Instruction);command.Parameters.AddWithValue("$state",job.DesiredState);command.Parameters.AddWithValue("$health",job.Health);command.Parameters.AddWithValue("$profile",(object?)job.ModelProfileId??DBNull.Value);command.Parameters.AddWithValue("$schedule",Serialize(job.Schedule));command.Parameters.AddWithValue("$next",job.NextOccurrence is null?DBNull.Value:FormatTimestamp(job.NextOccurrence.Value));command.Parameters.AddWithValue("$context",job.ContextPolicyJson);command.Parameters.AddWithValue("$created",FormatTimestamp(job.CreatedAt));command.Parameters.AddWithValue("$updated",FormatTimestamp(job.UpdatedAt));command.Parameters.AddWithValue("$version",job.Version);command.Parameters.AddWithValue("$kind",job.Kind);command.Parameters.AddWithValue("$conversation",(object?)job.ConversationId??DBNull.Value);await command.ExecuteNonQueryAsync(token).ConfigureAwait(false);}
        foreach(var account in job.AccountGrants) await InsertGrant(connection,transaction,"job_account_grants","account_id",job.OwnerPrincipalId,job.JobId,account,null,token).ConfigureAwait(false);
        foreach(var capability in job.CapabilityGrants) await InsertGrant(connection,transaction,"job_capability_grants","capability_id",job.OwnerPrincipalId,job.JobId,capability.Id,capability.Version,token).ConfigureAwait(false);
        foreach(var effect in job.SideEffectGrants) await InsertGrant(connection,transaction,"job_side_effect_grants","side_effect_class",job.OwnerPrincipalId,job.JobId,effect,null,token).ConfigureAwait(false);
