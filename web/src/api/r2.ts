...
export const r2Api = {
  ...
  developmentWorkspaces: (id: string) => request<R2Page<DevelopmentWorkspace>>(`/conversations/${encodeURIComponent(id)}/development-workspaces`),
  createDevelopmentTask: (conversationId: string, input: { name: string; workspaceId: string; commandProfile: 'repository.status'; arguments: string[] }) => mutate<DevelopmentTask>(`/conversations/${encodeURIComponent(conversationId)}/development-tasks`, 'POST', input, 'development-task'),
